...
class MemberForm(UserCreationForm):
    ...
    membership_id = forms.CharField(
        ...
        help_text="If you already have a membership id."
                  "Please enter it here")

    class Meta:
        model = CustomUser
        fields = ("username", "full_name",
                  "email", "password1", "password2", "membership_id")
...
class TimeSlotForm(forms.Form):
    timeslots = forms.ChoiceField(
        choices=[(f'{hour:02}:{minute:02}', f'{hour:02}:{minute:02}')
                 for hour in range(15, 22) for minute in range(0, 60, 15)])
...
class BookingForm(forms.ModelForm):
    ...
    def clean_booking_time(self):
        ...
        if (booking_date == date.today() and
                datetime.now().time() > booking_time):
            raise ValidationError(
